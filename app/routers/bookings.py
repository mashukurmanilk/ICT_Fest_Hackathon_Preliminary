...
# Application-level lock to serialize booking mutations (create / cancel)
# so that conflict, quota and double-cancel checks hold under concurrency.
...
def cancel_booking(
    ...
    with _booking_lock:
        # Re-check status under lock to prevent double-cancel
        db.refresh(booking)
